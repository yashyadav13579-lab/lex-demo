...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const rate = await checkRateLimit(`mut:${auth.user.id}:${getClientIp(request)}:drafts:update`, 90, 60_000)
...
export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
  ...
  const rate = await checkRateLimit(`mut:${auth.user.id}:${getClientIp(_request)}:drafts:delete`, 60, 60_000)
  if (!rate.allowed) return finalizeRequest(context, apiError(429, 'Too many requests. Try again shortly.', 'BAD_REQUEST'))
